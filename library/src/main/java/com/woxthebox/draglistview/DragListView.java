...
public class DragListView extends FrameLayout {
    ...
    private boolean handleTouchEvent(MotionEvent event) {
        mTouchX = event.getX();
        mTouchY = event.getY();
        Log.d("SwipeWords",""+event.getAction());
        if (isDragging()) {
            switch (event.getAction()) {

//                case MotionEvent.ACTION_DOWN:
//                    Log.d("SwipeWords","DragListView.ACTION_DOWN is invoked");
  //                  break;
                case MotionEvent.ACTION_DOWN:
                case MotionEvent.ACTION_MOVE:
                    mRecyclerView.onDragging(event.getX(), event.getY());
                    //Log.d("SwipeWords","DragListView.ACTION_MOVE is invoked");
                    break;

                    //Log.d("SwipeWords","DragListView.ACTION_UP is invoked");
                ...
                case MotionEvent.ACTION_CANCEL:
                    Log.d("SwipeWords","MotionEvent.ACTION_CANCEL??");

                    mRecyclerView.onDragEnded();
                    //Log.d("SwipeWords","DragListView.ACTION_UP/CANCEL is invoked");
//                    break;
            }
            return true;
        }
    ...
    public void setLayoutManager(LessonsLayoutManager layout) {
        mRecyclerView.setLayoutManager(layout);
    }
    ...
    public void setCustomDragItem(DragItem dragItem) {
        ...
        mRecyclerView.setDragItem(mDragItem);
        Log.d("MyGridLayoutManager","Adding view to recycler view??");
        addView(mDragItem.getDragItemView());
    }
