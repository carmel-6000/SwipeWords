    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.d("SwipeWords","DragItemRecyclerView.MotionEven.ACTION_DOWN");
                mStartY = event.getY();
                break;
...
//        Log.d("SwipeWords","updateDragPositionAndScroll step 1");
  ...
  //      Log.d("SwipeWords","updateDragPositionAndScroll step 2");
...
//        Log.d("SwipeWords","updateDragPositionAndScroll step 3");
    }
    ...
    boolean startDrag(View itemView, long itemId, float x, float y) {
        ...
        //if drag is disabled, or it is the first item and cannot drag above top,
        //or it is the last item and cannot drag below last
...
//        }
        ...
        //Log.d("SwipeWords","startDrag before invalidate");
        //always reaches here
        invalidate();
        return true;
    }
    ...
    void onDragging(float x, float y) {
        if (mDragState == DragState.DRAG_ENDED) {
            Log.d("SwipeWords","onDragging onDragState==DragState.DRAG_ENDED?");
            return;
        }
        Log.d("SwipeWords","onDragging");
        ...
        if (!mAutoScroller.isAutoScrolling()) {
            Log.d("SwipeWords","updateDragPositionAndScroll is invoked");
            updateDragPositionAndScroll();
        }
    ...
    void onDragEnded() {
        Log.d("SwipeWords","onDragEnded is invoked");
        ...
        //Log.d("SwipeWords","mDisableReorderWhenDragging?"+(mDisableReorderWhenDragging ? "TRUE" :"FALSE"));
        //mDisableReorderWhenDragging is FALSE
        if (mDisableReorderWhenDragging) {
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                if (holder != null) {
                    Log.d("SwipeWords","holder is not null");
                    ...
                    Log.d("SwipeWords","holder is ~~ null");
                    onDragItemAnimationEnd();
                }
            }
        });
    ...
    int getDragPositionForY(float y) {
        ...
        if (pos == NO_POSITION) {
            Log.d("SwipeWords","pos is NO_POSITION?");
            pos = 0;
        }
        return pos;
    }

    void addDragItemAndStart(float y, WordItem item, long itemId) {
